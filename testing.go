...
func setupWithCleanup[T interface{ Close() error }](tb testing.TB, start func(context.Context) (T, error), closeTarget string) T {
	...
	tb.Cleanup(func() {
		if err := value.Close(); err != nil {
			tb.Errorf("spanemuboost: failed to close %s: %v", closeTarget, err)
		}
	})
...
func SetupEmulator(tb testing.TB, options ...Option) *Emulator {
	return setupWithCleanup(tb, func(ctx context.Context) (*Emulator, error) {
		return RunEmulator(ctx, options...)
	}, "emulator")
}

func setupOmni(tb testing.TB, options ...Option) Runtime {
	return setupWithCleanup(tb, func(ctx context.Context) (Runtime, error) {
		return runOmni(ctx, options...)
	}, "omni")
}

func setupOmniWithClients(tb testing.TB, options ...Option) *RuntimeEnv {
	return setupWithCleanup(tb, func(ctx context.Context) (*RuntimeEnv, error) {
...
func SetupEmulatorWithClients(tb testing.TB, options ...Option) *Env {
	return setupWithCleanup(tb, func(ctx context.Context) (*Env, error) {
		return RunEmulatorWithClients(ctx, options...)
	}, "env")
}
